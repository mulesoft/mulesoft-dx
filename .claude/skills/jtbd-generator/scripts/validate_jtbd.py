...
class JobValidator:
    ...
    def load_api_spec(self, api_urn: str) -> Optional[Dict[str, Any]]:
        ...
        api_folder = self.api_specs_root / folder_name
...
def find_api_specs_root(job_file: Path) -> Path:
    ...
    # Walk up to find the repository root (contains multiple API folders)
    current = job_file.parent

    # Go up until we find a directory containing api-manager/
    for _ in range(5):  # Limit search depth
        if (current / 'api-manager').exists():
            return current
        current = current.parent

    # Fallback: assume parent of parent (skills/ -> api-manager/ -> root)
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/validate_jtbd.py <jtbd-file.md> [api-specs-root]")
        ...
        print("\nExamples:")
        print("  python scripts/validate_jtbd.py skills/deploy-api-with-rate-limiting/SKILL.md .")
        print("  python scripts/validate_jtbd.py job.md /path/to/api-specs")
        sys.exit(1)
