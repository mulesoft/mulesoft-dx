...
- `AsyncAPI` - An API specification allows you to create machine-readable definitions of your asynchronous APIs.
...
- `GraphQL API` - A schema definition that contains the object types and definitions that are used to interact with the API through the GraphQL specification. GraphQL enables you to query an API that supports this language in a much more flexible way than REST.
...
- `MCP` - Model Context Protocol asset that provides the protocols and specifications for seamless integration between different AI systems and tools.
- `Agent` - Agent asset that defines an intelligent agent, including its capabilities, actions, and metadata for discovery and reuse.
- `LLM` - Large Language Model asset that defines model metadata, capabilities, parameters, and usage policies for discovery and governance across Exchange.
...
- `fields`: A JSON array whose items are objects with properties key and value, which represent custom fields. The custom field must be configured in your organization. The example of uploading a custom asset includes custom fields.
...
- `categories`: A JSON array whose items are objects with properties key and value, which represent custom fields. The custom field must be configured in your organization. The example of uploading a custom asset includes custom fields.
...
To learn more about the documentation zip file, download the [example documentation zip file](https://github.com/mulesoft/exchange-xapi/blob/master/docs/resources/multipart-examples-files/docs.zip).
...
In the multipart form, Exchange accepts categories and custom fields in this format or in the JSON format explained in the previous section.
...
This example creates an asset of type `ASYNCAPI`.

Download the file used in this example [here](https://github.com/mulesoft-labs/exchange-documentation-samples/tree/master/asyncapi).

Replace `/file-path/async-api-request-api.zip` with the local path of the downloaded file.

Replace the following with the details needed for your upload:
...
curl -v \
  ...
  -F 'name=ASYNCAPI asset name' \
  -F 'description=Description of the asset' \
  -F 'properties.mainFile=async-trade-request-api.yaml' \
'evented-api.zip=@/file-path/async-api-request-api.zip' \
https://anypoint.mulesoft.com/exchange/api/v2/organizations/:organizationId/assets/:groupId/:assetId/:version
```

_Note: This example uploads a file with classifier _`evented-api`_ and _`zip`_ packaging. You may instead upload a single file with packaging _`.yaml`_ and the _`type`_ specified._
...
_Note: This example uploads a file with classifier _`oas`_ and _`zip`_ packaging. You may instead upload a single _`oas`_ file with packaging _`.yaml`_ or _`.json`_._
...
This example creates an asset of type `Graphql`.

Download the file used in this example [here](https://github.com/mulesoft-labs/exchange-documentation-samples/blob/master/graphql/example.graphql).
...
- `description`
- `mainFile`
- `file-path`: replace with the local path of the required file
...
_Note: This example uploads a file with classifier _`graphql`_ ._
...
This example creates an asset of type `MCP`.

Download the file used in this example [here](https://docs.mulesoft.com/exchange/to-create-an-asset#create-mcp). This file is optional.
...
_Note: The type field is specified because it can not be inferred from a file classifier. Also the platform attribute is optional and can be inferred from the mcp-metadata file._

```
curl -v \
  -H 'Authorization: bearer ANYPOINT_TOKEN' \
  -H 'x-sync-publication: true' \
  -F 'name=MCP asset name' \
  -F 'description=Description of the asset' \
  -F 'type=mcp' \
  -F 'files.mcp-metadata.json=@/:file-path/example-mcp.json' \
  -F 'properties.platform="mulesoft"' \
https://anypoint.mulesoft.com/exchange/api/v2/organizations/:organizationId/assets/:groupId/:assetId/:version
```

# Agent

This example creates an asset of type `Agent`.

Download the file used in this example [here](https://a2a-protocol.org/dev/specification/). This file is optional.
...
_Note: The type field is specified because it can not be inferred from a file classifier. Also the platform attribute is optional and can be inferred from the agent-metadata file._

```
curl -v \
  -H 'Authorization: bearer ANYPOINT_TOKEN' \
  -H 'x-sync-publication: true' \
  -F 'name=Agent asset name' \
  -F 'description=Description of the asset' \
  -F 'type=agent' \
  -F 'files.agent-metadata.json=@/:file-path/example-agent-card.json' \
  -F 'properties.platform="mulesoft"' \
https://anypoint.mulesoft.com/exchange/api/v2/organizations/:organizationId/assets/:groupId/:assetId/:version
```
...
This example creates an asset of type `LLM`.
...
### Impersonate asset creator

When creating an asset using the api you can choose to assign another **member of the organization** as the asset creator. This is done by using the `x-user-id` header. The header value should be the user id of the user that will be set as the asset creator.

```
curl --request POST 'https://anypoint.mulesoft.com/exchange/api/v2/organizations/:organizationId/assets/:groupId/:assetId/:version' \
  --header 'Authorization: bearer ANYPOINT_TOKEN' \
  --header 'x-sync-publication: true' \
  --header 'x-user-id: another_org_member_user_id' \
  --form 'name="Comprehensive Test"' \
  --form 'type="custom"' \
  --form 'status="published"' \
  --form 'x-sync-publication="true"'
```

### Role Assignment with connected apps

When creating an asset using a **connected app** (exclusively) you can choose whether to assign roles or not to the connected app owner. By default, this behavior is defined by the `assetRoleAssignmentSettings.assignRoleToConnectedApps` setting. You can use the `x-assign-roles` header to override the setting value of the organization.

If the setting value is set to "true" it will assign roles to the connected app creator and if the setting is set to "false" it will not.

When combined with the `x-user-id` header, the `x-assign-roles` header will define if roles are assigned or not to the impersonation user passed on the x-user-id header.
```
curl --location --request POST 'https://anypoint.mulesoft.com/exchange/api/v2/organizations/:organizationId/assets/:groupId/:assetId/:version' \
  --header 'Authorization: bearer YOUR_TOKEN' \
  --header 'x-sync-publication: true' \
  --header 'x-assign-roles: true' \
  --form 'name="Comprehensive Test"' \
  --form 'type="custom"' \
  --form 'status="published"' \
  --form 'x-sync-publication="true"'
