...
**⚠️ NEVER edit `target/`.** `target/` is Maven build output generated by `agent-network project build` — a copy of the packaged artifact, not source. Editing files there (including any `agent-network.yaml`, `.agent`, or policy config that appears under `target/`) does **nothing** to the project; the next build overwrites it. **All configuration edits — including adding a connection policy (e.g. client-id enforcement), changing auth, or editing a card — belong in the source `agent-network.yaml` / `brokers/*.agent`, never in `target/`.** Likewise never touch `.mvn/`, `.vscode/`, `.cursor/`, or other IDE/build folders. If you can't find a setting in the source files, ask the user where the source lives — don't reach into `target/`.
...
### `supportedInterfaces` — multiple transport/URL bindings on one card

A card can advertise more than one transport binding via `metadata.interfaces.a2a.card.supportedInterfaces` (also valid on `a2a_v03`). It's an **array of interface objects**, each with:

| Field | Meaning | Valid values |
|---|---|---|
| `url` | URL where this interface is available | valid URI string |
| `protocolBinding` | protocol binding for this interface | `JSONRPC`, `GRPC`, `HTTP+JSON` |
| `protocolVersion` | A2A protocol version for this interface | e.g. `"1.0"` (quote it) |
| `tenant` | optional tenant id sent when calling the agent | any string |

All four fields are individually optional per the schema, but a binding is only useful with at least `url` + `protocolBinding`.

**When the user asks to "support both HTTP+JSON and JSONRPC," write exactly two objects — do not iterate or guess.** Concrete shape:
...
Keys are literal: `protocolBinding` (not `binding`/`protocol`), `HTTP+JSON` (with the `+`, not `HTTP-JSON` or `JSON`), `JSONRPC` (one word, no hyphen). `protocolVersion` is a **quoted string** (`"1.0"`), never a bare number.

**⚠️ protocolVersion must match the deployed agent's real runtime version.** Per the YAML reference: *"A mismatch causes the platform to apply the wrong Agent Card policy, which prevents the agent card URL from being rewritten to the egress proxy and results in a `401 Unauthorized` error at runtime rather than a version error."* So a `401` on a downstream call is a symptom of a wrong `protocolVersion`, not necessarily an auth-config problem.
...
| Undeploy from runtime | 10 | `agent-network project undeploy` | `undeploy_network_project` | Doc link |
| Unpublish from Exchange | 11 | `agent-network project unpublish` | `unpublish_network_project` | Doc link |
...
| Step 10 Undeploy | `anypoint-cli-agent-fabric-plugin agent-network project undeploy` | `mcp__mulesoft-agent-fabric-manager__undeploy_network_project` |
| Step 11 Unpublish | `anypoint-cli-agent-fabric-plugin agent-network project unpublish` | `mcp__mulesoft-agent-fabric-manager__unpublish_network_project` |
...
Undeploys the Agent Network application, stopping all API instances/applications and removing the deployed runtime resources. Undeploy does **not** touch Exchange — the published assets remain; you can redeploy. (Public CLI command as of plugin **v1.3.0**.)

**CLI:** `anypoint-cli-agent-fabric-plugin agent-network project undeploy [flags]`. Useful flags:
- `--path <projectPath>` — discover deployed resources from the project lock file. **Requires an environment.**
- `--gav <groupId:assetId:version>` — discover deployed resources remotely (mutually exclusive with `--path`). Requires an environment.
- `--environment <name>` (or `ANYPOINT_ENV`) — required to resolve what to undeploy.
- `--api-version <value>` (default `v1`) — API version used to discover remote resources when using `--gav`.
- `--dry-run` — preview the resources that would be undeployed without deleting anything. **Recommend running this first.**
- `--process-timeout <ms>` (default `900000`), `--request-timeout <ms>` (default `60000`), `--json`.

**MCP:** `undeploy_network_project`.

**Anypoint Code Builder:** right-click a project file → *Undeploy Agent Network*, or Command Palette → *MuleSoft: Undeploy Agent Network*, then confirm *Undeploy* in the dialog. **MuleSoft Vibes:** ask it in plain language (e.g. *"Undeploy my agent network project from the production environment."*).

### Step 11 — Unpublish (optional; soft-delete default, `--hard-delete` irreversible)

Removes the Agent Network assets from Anypoint Exchange. (Public CLI command as of plugin **v1.3.0**.)

**Ordering rule (correctness):** **Undeploy before you unpublish.** Unpublishing assets that are still deployed leaves orphaned runtime references. (Docs: *"To prevent orphaned runtime references, undeploy your agent network before unpublishing."*)

**Soft-delete vs hard-delete:**
- **Default = soft-delete.** The asset version is deleted and can no longer be used, but its **GAV stays reserved** (cannot be claimed by a new asset) and the removal **cannot be restored/undeleted**. Exchange waits **7 days**, then permanently (hard) deletes it. If you **republish before** that hard delete, Exchange uses the **next available patch version** (e.g. `1.0.x`).
- **`--hard-delete`** — permanently removes assets immediately and **frees the GAV** for reuse by another asset. **Irreversible.** Assets with dependent versions, API instances, or policies applied to managed APIs must have those removed first.

**CLI:** `anypoint-cli-agent-fabric-plugin agent-network project unpublish [flags]`. Useful flags:
- `--path <projectPath>` — reads published assets from the project lock file. Select an environment to enable the active-API-instances safety check.
- `--gav <groupId:assetId:version>` — discover assets remotely (mutually exclusive with `--path`). Use `--environment` to enable the active-API-instances safety check.
- `--environment <name>` — enables the safety check that blocks unpublishing assets still backing active API instances.
- `--hard-delete` — see above. Irreversible.
- `--dry-run` — preview the Exchange assets that would be unpublished without deleting anything. **Recommend running this first.**
- `--api-version <value>` (default `v1`), `--json`.

**MCP:** `unpublish_network_project`.

**Anypoint Code Builder:** right-click a project file → *Unpublish Agent Network*, or Command Palette → *MuleSoft: Unpublish Agent Network*, then confirm *Unpublish* in the dialog. **MuleSoft Vibes:** ask it in plain language (e.g. *"Unpublish my agent network project from Exchange."*).

Because unpublish (and `--hard-delete` especially) is destructive and hard to reverse, **confirm with the user before running it without `--dry-run`.**
...
- **Step 10 — undeploy:** Point at <https://docs.mulesoft.com/anypoint-code-builder/af-deploy-agent-network-targets> (§ Undeploy).
- **Step 11 — unpublish:** Point at <https://docs.mulesoft.com/anypoint-code-builder/af-publish-agent-network-assets> (§ Unpublish).
...
- **Don't auto-undeploy or auto-unpublish.** Steps 10 (undeploy) and 11 (unpublish) are destructive/hard-to-reverse — run only on explicit request, prefer `--dry-run` first, undeploy before unpublish, and confirm before any `--hard-delete`.
- **Don't edit `target/`** (or `.mvn/`, `.vscode/`, `.cursor/`). It's build output — all edits go in source `agent-network.yaml` / `brokers/*.agent`.
