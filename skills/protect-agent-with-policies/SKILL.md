...
inputs:
  types:
    value: "agent"
    description: Filter for agent assets only
outputs:
  - name: groupId
    path: $[*].groupId
    labels: $[*].name
    description: Group ID of the selected agent asset
  - name: assetId
    path: $[*].assetId
    description: Asset ID of the selected agent asset
  - name: assetVersion
    path: $[*].version
    description: Version of the selected agent asset
```
...
inputs:
  ...
  instanceLabel:
    userProvided: true
    description: A human-readable label for this API instance (e.g., "my-agent-v1")
    example: "my-agent-v1"
  technology:
    value: "flexGateway"
  ...
  endpoint.uri:
    userProvided: true
    optional: true
    description: The upstream backend URL for the agent. Ask the user if they want to provide it now or configure it later.
    example: "https://backend.example.com/agent/v1"
outputs:
...
inputs:
  ...
  family:
    value: "agentic"
    description: Filter for agent (agentic) instances only
outputs:
  - name: environmentApiId
    path: $.assets[*].apis[*].id
    labels: $.assets[*].apis[*].autodiscoveryInstanceName
    description: The agent instance ID in API Manager
```
